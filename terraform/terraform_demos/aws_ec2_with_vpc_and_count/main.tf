...
resource "aws_network_interface_sg_attachment" "sg_attachment" {
  ...
  network_interface_id = aws_network_interface.primary_eni[count.index].id
}

# Create public IP (Elastic IP)
resource "aws_eip" "ec2_ipaddress" {
  count = 2
  network_interface = aws_network_interface.primary_eni[count.index].id
...
resource "aws_instance" "web" {
  ...
  tags = {
    Name = format("Webserver-%s", count.index+1)
  }
}
